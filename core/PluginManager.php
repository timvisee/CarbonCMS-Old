...
use core\Database;
use core\event\plugin\PluginLoadEvent;
...
class PluginManager {
    ...
    /**
     * @var string $plugins_dir Plugin's directory
    ...
    public function getPluginCache($plugin_name) {
        return new CacheHandler($this->cache->getCacheDir() . 'plugin/' . $plugin_name . '/');
    }
    ...
    public function loadPlugin($plugin_dir) {
        // Check if the plugin's folder exist
        if(!is_dir($plugin_dir))
            return;
        
        // Force the plugin's dir to end with a directory separator
        $plugin_dir = rtrim($plugin_dir, '/\\') . '/';
        
        // Check if there's already a plugin loaded from this directory
        if($this->isPluginLoadedFromDir($plugin_dir))
            return;
        
        // Get the plugin's settings
        $plugin_settings_file = $plugin_dir . 'plugin.ini';
        $plugin_settings = new PluginSettings($plugin_settings_file);
        
        // Make sure the plugin's settings file is valid
        if(!$plugin_settings->isValid()) {
            // Throw an exception
            throw new \Exception('Carbon CMS: Unable to loadLocalesList the plugin from \'' . $plugin_dir . '\', this plugin has an invalid \'plugin.ini\' file!');
            die();
        }
        ...
        // Check if the main file path and the main class name are valid
        if($plugin_main_file == null || $plugin_main_file == '') {
            // Throw an exception
            throw new \Exception('Carbon CMS: Unable to loadLocalesList the plugin \'' . $plugin_display . '\', the key \'plugin.main\' is invalid inside the \'plugin.ini\' file!');
            die();
        } else if($plugin_main_class == null || $plugin_main_class == '') {
            // Throw an exception
            throw new \Exception('Carbon CMS: Unable to loadLocalesList the plugin \'' . $plugin_display . '\', the key \'plugin.main_class\' is invalid inside the \'plugin.ini\' file!');
            die();
        }
        ...
        if(!file_exists($plugin_main_file)) {
            // Throw an exception
            throw new \Exception('Carbon CMS: Unable to loadLocalesList the plugin \'' . $plugin_display . '\', the main file \'' . $plugin_main_file . '\' does not exist!');
            die();
        }
        
        // Load the plugin's file
        include($plugin_main_file);
        
        // Check if the plugin's main class is loaded
     ...
     * @param string $plugin_name Plugin's unique name
     * @return mixed Plugin or null when no plugin was found
     */
     ...
     * @param string $plugin_name Plugin's unique name to check for
     * @return boole True if this plugin was loaded, false if it wasn't loaded or if it wasn't found
     */
    ...
    public function getEnabledPlugins() {
        ...
        // Loop through each plugin and check if it's enabled, if so add it to the list
    ...
    /**
     * Enable a plugin
     * @param Plugin $plugin Plugin to enable
     * @throws \Exception
     */
